...
const generateRandomSnake = () => {
  ...
  const snakeLength = Math.floor(Math.random() * (MAX_SNAKE_LENGTH - MIN_SNAKE_LENGTH) + MIN_SNAKE_LENGTH)
  ...
  const stepInCourseOfWay = (way = DIRECTIONS[Math.floor(Math.random() * 4)], course = FORWARD, currentPoint) => {
    let direction = directionsMap[`${way}_${course}`]
    let nextPoint = {
      x: currentPoint.x + direction.x,
      y: currentPoint.y + direction.y
    }
    return [nextPoint, direction.turn]
  }

  let i = 1
  while (i < snakeLength) {
    [nextPointG, turn] = stepInCourseOfWay(
                                            turn,
